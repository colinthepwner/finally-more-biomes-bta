...
public class BetterOPlentyClient implements ModelEntrypoint {
	...
	private void bindGroundCoverModels(BlockModelDispatcher dispatcher) {
		...
		for (int i = 0; i < blocks.size(); i++) {
			...
			if (blocks.get(i) == BOPPlants.REED) {
				dispatcher.addDispatch(new BlockModelWaterReed<>(blocks.get(i))
					.withTextures(textures.get(i)));
			} else if (takesGrassOffset(blocks.get(i))) {
				dispatcher.addDispatch(new BlockModelCrossedSquaresShifted<>(blocks.get(i))
					.withTextures(textures.get(i)));

			} else if (blocks.get(i) == BOPPlants.BERRY_BUSH) {
				dispatcher.addDispatch(new BlockModelOverlayCross<>(blocks.get(i),
						"betteroplenty:block/berrybush_berry", "betteroplenty:block/item_berrybush")
					.withTextures(textures.get(i)));
			} else if (blocks.get(i) == BOPPlants.SHRUB) {
				dispatcher.addDispatch(new BlockModelOverlayCross<>(blocks.get(i),
						"betteroplenty:block/shrub_branch", "betteroplenty:block/item_shrub")
					.withTextures(textures.get(i)));
			} else {
