...
import net.minecraft.client.render.worldtype.WorldTypeFXDrift;
...
public class WorldTypeFXPromised extends WorldTypeFXDrift {

	public WorldTypeFXPromised(@NotNull WorldType worldType) {
		super(worldType);
	}
	...
	public Vector3fc getFogColor(@NotNull World world, double x, double y, double z,
		...
		if (!Colorizers.fog.isEnabled()) {
			...
			if (target == null) {
				float dayProgress = MathHelper.cos(celestialAngle * (float) Math.PI * 2.0F) * 2.0F + 0.5F;
				dayProgress = MathHelper.clamp(dayProgress, 0.0F, 1.0F);
				float r = 1.0F * (dayProgress * 3.94F + 0.06F);
				float g = 0.73725490196F * (dayProgress * 0.94F + 0.06F);
				float b = 0.25882352941F * (dayProgress * 0.91F + 0.09F);
				target = new Vector3f(MathHelper.clamp(r, 0.0F, 1.0F), g, b);
			}
		}
