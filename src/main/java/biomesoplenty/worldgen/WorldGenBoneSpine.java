...
public class WorldGenBoneSpine extends BOPWorldFeature {
	...
	private static void placeRibRing(World world, int x, int y, int z) {
		...
		setBlockAndMetadata(world, x - 1, y, z, BOPBones.MEDIUM.id(), AXIS_X);
		setBlockAndMetadata(world, x + 1, y, z, BOPBones.MEDIUM.id(), AXIS_X);
		setBlockAndMetadata(world, x, y, z - 1, BOPBones.MEDIUM.id(), AXIS_Z);
		setBlockAndMetadata(world, x, y, z + 1, BOPBones.MEDIUM.id(), AXIS_Z);

		setBlockAndMetadata(world, x - 2, y, z, BOPBones.SMALL.id(), AXIS_X);
		setBlockAndMetadata(world, x + 2, y, z, BOPBones.SMALL.id(), AXIS_X);
		setBlockAndMetadata(world, x, y, z - 2, BOPBones.SMALL.id(), AXIS_Z);
		setBlockAndMetadata(world, x, y, z + 2, BOPBones.SMALL.id(), AXIS_Z);
	}

	static final int AXIS_Z = 1;

	static final int AXIS_X = 2;
}
