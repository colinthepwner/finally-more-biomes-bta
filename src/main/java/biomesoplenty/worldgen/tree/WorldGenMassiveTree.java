...
public class WorldGenMassiveTree extends BOPWorldFeature {
	...
	private final int[] scratchNodeStart = new int[3];
	private final int[] scratchNodeEnd = new int[3];
	private final int[] scratchBranchBase = new int[3];
	private final int[] scratchLineDelta = new int[3];
	private final int[] scratchLineStep = new int[3];
	private final int[] scratchPlaceDelta = new int[3];
	private final int[] scratchPlaceStep = new int[3];
	private final int[] scratchLayerBase = new int[3];
	private final int[] scratchLayerPos = new int[3];

	private static final int LEAF_PERMANENT = 1;

	private static final int LOG_AXIS_Y = 0;
	private static final int LOG_AXIS_X = 2;
	private static final int LOG_AXIS_Z = 1;
		...
		while (var6 >= 0) {
			...
			if (var8 < 0.0F) {
				...
				for (double var9 = 0.5D; var7 < var1; ++var7) {
					...
					int[] var17 = this.scratchNodeStart;
					var17[0] = var15;
					var17[1] = var3;
					var17[2] = var16;
					int[] var18 = this.scratchNodeEnd;
					var18[0] = var15;
					var18[1] = var3 + leafDistanceLimit;
					var18[2] = var16;

					if (this.checkBlockLine(var17, var18) == -1) {
						int[] var19 = this.scratchBranchBase;
						var19[0] = basePos[0];
						var19[1] = basePos[1];
						var19[2] = basePos[2];
	...
	void genTreeLayer(int par1, int par2, int par3, float par4, byte par5, int par6) {
		...
		int[] var10 = this.scratchLayerBase;
		var10[0] = par1;
		var10[1] = par2;
		var10[2] = par3;
		int[] var11 = this.scratchLayerPos;
		int var12 = -var7;
		int var13 = -var7;
		...
		for (var11[par5] = var10[par5]; var12 <= var7; ++var12) {
			...
			while (var13 <= var7) {
				...
				if (var15 > par4 * par4) {
					...
					int var14 = getBlockId(worldObj, var11[0], var11[1], var11[2]);

					if (var14 != 0 && var14 != Blocks.LEAVES_OAK.id()) {
						++var13;
					} else {

						setBlockAndMetadata(worldObj, var11[0], var11[1], var11[2], par6, LEAF_PERMANENT);
						++var13;
					}
				}
			}
	...
	float leafSize(int par1) {
		return par1 >= 0 && par1 < leafDistanceLimit
			? (par1 != 0 && par1 != leafDistanceLimit - 1 ? 3.0F : 2.0F)
			: -1.0F;
	}
	...
	void generateLeafNode(int par1, int par2, int par3) {
		...
		for (int var5 = par2 + leafDistanceLimit; var4 < var5; ++var4) {
			...
			this.genTreeLayer(par1, var4, par3, var6, (byte) 1, Blocks.LEAVES_OAK.id());
		}
	}

	void placeBlockLine(int[] par1ArrayOfInteger, int[] par2ArrayOfInteger, int par3) {
		int[] var4 = this.scratchPlaceDelta;
		byte var5 = 0;
		byte var6;
		...
		if (var4[var6] != 0) {
			...
			int[] var14 = this.scratchPlaceStep;
			int var15 = 0;
			...
			for (int i = -2; i < 3; i++) {
				...
				for (int j = -2; j < 3; j++) {
					if (!(i == -2 && j == -2) && !(i == -2 && j == 2)
						...
						var5[2] = var4 + j;
						var6[2] = var4 + j;
						this.placeBlockLine(var5, var6, Blocks.LOG_OAK.id());
					}
				}
			}
		}
	...
	void generateLeafNodeBases() {
		...
		for (int[] var3 = new int[] {basePos[0], basePos[1], basePos[2]}; var1 < var2; ++var1) {
			...
			if (this.leafNodeNeedsBase(var6)) {
				this.placeBlockLine(var3, var5, Blocks.LOG_OAK.id());
			}
		}
	}

	int checkBlockLine(int[] par1ArrayOfInteger, int[] par2ArrayOfInteger) {
		int[] var3 = this.scratchLineDelta;
		byte var4 = 0;
		byte var5;
		...
		if (var3[var5] == 0)
			...
			int[] var13 = this.scratchLineStep;
			int var14 = 0;
			int var15;
			...
			for (var15 = var3[var5] + var8; var14 != var15; var14 += var8) {
				...
				int var16 = getBlockId(worldObj, var13[0], var13[1], var13[2]);

				if (var16 != 0 && var16 != Blocks.LEAVES_OAK.id()) {
					break;
				}
			}
	...
	boolean validTreeLocation() {
		...
		int var3 = getBlockId(worldObj, basePos[0], basePos[1] - 1, basePos[2]);

		if (!Blocks.hasTag(var3, BlockTags.GROWS_TREES))
			return false;
		else {
	...
	@Override
	public void init(double par1, double par3, double par5) {
