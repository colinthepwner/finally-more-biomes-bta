...
public class WorldGenPromisedWillowTree extends BOPWorldFeature {

	@Override
	public boolean generate(World par1World, Random par2Random, int par3, int par4, int par5) {
		final int leavesId = Blocks.LEAVES_OAK.id();
		final int logId = Blocks.LOG_OAK.id();
		final int leafData = BlockLogicLeavesBase.setPermanent(0, true);
		...
		for (var6 = par2Random.nextInt(8) + 6; getBlockMaterial(par1World, par3, par4 - 1, par5) == Materials.WATER; --par4) {
			;
		}
		...
		if (par4 >= 1 && par4 + var6 + 1 <= worldHeight) {
			...
			if (!var7)
				...
				if ((var8 == BOPPromisedLand.HOLY_GRASS.id() || var8 == BOPPromisedLand.HOLY_DIRT.id())
					&& par4 < worldHeight - var6 - 1) {

					setBlock(par1World, par3, par4 - 1, par5, BOPPromisedLand.HOLY_DIRT.id());
					int var13;
					int var16;
						...
						for (var12 = par3 - var11; var12 <= par3 + var11; ++var12) {
							...
							for (int var14 = par5 - var11; var14 <= par5 + var11; ++var14) {
								...
								if ((Math.abs(var13) != var11 || Math.abs(var15) != var11 || par2Random.nextInt(2) != 0 && var10 != 0) && !isOpaqueCube(par1World, var12, var16, var14)) {
									...
									setBlockAndMetadata(par1World, var12, var16, var14, leavesId, leafData);
								}
							}
						}
					}
							...
							for (var13 = par5 - var11; var13 <= par5 + var11; ++var13) {
								if (getBlockId(par1World, var12, var16, var13) == leavesId) {
									...
									if (par2Random.nextInt(4) == 0 && getBlockId(par1World, var12 - 1, var16, var13) == 0) {
										generateSkirt(par1World, var12 - 1, var16, var13, leavesId, leafData);
									}
									...
									if (par2Random.nextInt(4) == 0 && getBlockId(par1World, var12, var16, var13 - 1) == 0) {
										generateSkirt(par1World, var12, var16, var13 - 1, leavesId, leafData);
									}
	...
	private static void generateSkirt(World world, int x, int y, int z, int leavesId, int leafData) {
		setBlockAndMetadata(world, x, y, z, leavesId, leafData);
		int remaining = 24;

		while (true) {
			--y;

			if (getBlockId(world, x, y, z) != 0 || remaining <= 0)
				return;
